        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('mahasiswa.matakuliah.update', $mhsMatkul->id) }}" method="post" novalidate>
                        @csrf
                        @method('put')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="mahasiswa" class="form-control @error('mahasiswa') is-invalid @enderror">
                                        ...
                                        @foreach ($mahasiswa as $item)
                                            <option value="{{ $item->id }}" {{ $mhsMatkul->mahasiswa_id == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                        @endforeach
                                    </select>
                                    @error('mahasiswa')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="matakuliah" class="form-control @error('matakuliah') is-invalid @enderror">
                                        ...
                                        @foreach ($matakuliah as $item)
                                            <option value="{{ $item->id }}" {{ $mhsMatkul->matakuliah_id == $item->id ? 'selected' : '' }}>{{ $item->nama_matakuliah }}</option>
                                        @endforeach
                                    </select>
                                    @error('matakuliah')
                                        <span class="invalid-feedback">
